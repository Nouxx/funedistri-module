...
| **Locked address book** (B2B user selects only company addresses; shipping←Delivery, billing←Invoice; strict separation; no create/edit at checkout or portal) | `res_users.py` (`_coffin_is_b2b_user`) | `address_lock_templates.xml` (hide delivery + billing "Add Address") | `address_lock.py` (block checkout + portal create/edit; validate selected by type; auto-assign both; block-on-missing) | seed: company Delivery + Invoice addresses | — | `test_address_lock.py` | 0005 | 5+5b |
