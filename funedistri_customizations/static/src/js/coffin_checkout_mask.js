...
// Step 4 fix — keep the checkout "Confirm" button usable for a price-masked
// SALESMAN.
//
...
//   so the native code runs fine — which is why the bug looked Salesman-specific.)
//
// THE FIX:
//   Patch `_updateCartSummary` to no-op when the totals table is absent. There is
//   genuinely nothing to update for a masked Salesman, so skipping is correct AND
//   removes the throw, letting `_enableMainButton()` run. Everyone else keeps the
//   table and defers to the native implementation unchanged.
